...
glmnet_mod = function(index_trait, alpha, dfmax = NULL, mod.family, feature = "all"){
  if(feature == "fc"){
  ...
  mod = glmnet(x_trn, y_trn, family = mod.family, alpha = alpha, dfmax = dfmax, lambda = cvfit$lambda.min)
  ...
  return(list(mod = mod, predict = mod.pred, err = sum((y_tst-mod.pred)^2), threshold = threshold, confusion.matrix = conf.mat$table, ratio = conf.mat$byClass))
}
```
...
```{r}
set.seed(15)
mod.lasso.130.sc = glmnet_mod(130, 1, mod.family = "binomial", feature = "sc")
...
mod.lasso.130.fc = glmnet_mod(130, 1, mod.family = "binomial", feature = "fc")
...
# doesn't work
mod.lasso.130.all = glmnet_mod(130, 1, mod.family = "binomial", feature = "all")
...
```{r}
set.seed(15)
mod.lasso.130.sc = glmnet_mod(130, 0.8, mod.family = "binomial", feature = "sc")
# lasso model with only sc
mod.lasso.130.sc$mod
# confusion matrix
mod.lasso.130.sc$confusion.matrix
...
mod.lasso.130.fc = glmnet_mod(130, 0.8, mod.family = "binomial", feature = "fc")
# lasso model with only fc
mod.lasso.130.fc$mod
# confusion matrix
mod.lasso.130.fc$confusion.matrix
...
mod.lasso.130.all = glmnet_mod(130, 0.8, mod.family = "binomial", feature = "all")
# lasso model will all features
mod.lasso.130.all$mod
# confusion matrix
mod.lasso.130.all$confusion.matrix
...
```{r}
mod.lasso.131.sc = glmnet_mod(131, 1, mod.family = "gaussian", feature = "sc")
# lasso model with only sc
mod.lasso.131.sc$mod
# mse
mod.lasso.130.sc$err

mod.lasso.131.fc = glmnet_mod(130, 1, mod.family = "gaussian", feature = "fc")
# lasso model with only fc
...
mod.lasso.131.all = glmnet_mod(130, 1, mod.family = "gaussian", feature = "all")
# lasso model will all features
...
```{r}
mod.ridge.130.sc = glmnet_mod(130, 0, mod.family = "binomial", feature = "sc")
...
mod.ridge.130.fc = glmnet_mod(130, 0, mod.family = "binomial", feature = "fc")
...
mod.ridge.130.all = glmnet_mod(130, 0, mod.family = "binomial", feature = "all")
...
```{r}
mod.ridge.131.sc = glmnet_mod(131, 0, mod.family = "gaussian", feature = "sc")
# ridge model with only sc
...
mod.ridge.131.fc = glmnet_mod(131, 0, mod.family = "gaussian", feature = "fc")
# ridge model with only fc
...
mod.ridge.131.all = glmnet_mod(130, 0, mod.family = "gaussian", feature = "all")
# ridge model with all features
...
```{r}
library(e1071)
svm.fit = function(index_trait, type = "C-classification", feature = "all"){
  if(feature == "fc"){
  ...
  if(feature == "fc"){
    mod.svm = svm(Depressive_Ep~FCfeature_1+FCfeature_2+FCfeature_3, data = df_train, kernel="radial", type = type, cost = 40000)
    summary(mod.svm)
    svm.pred = predict(mod.svm, df_test[,c(1:3)])
  }else if(feature == "sc"){
    mod.svm = svm(Depressive_Ep~SCfeature_1+SCfeature_2+SCfeature_3, data = df_train, kernel="radial", type = type, cost = 40000)
    summary(mod.svm)
    svm.pred = predict(mod.svm, df_test[,c(1:3)])
  }else{
    mod.svm = svm(Depressive_Ep~FCfeature_1+FCfeature_2+FCfeature_3+SCfeature_1+SCfeature_2+SCfeature_3, data = df_train, kernel="radial", type = type, cost = 40000)
    summary(mod.svm)
...
```{r}
svm.fit(130)
svm.fit(130, feature = "sc")
svm.fit(130, feature = "fc")
```

## SVM for 131

```{r}
svm.fit(131, type = "eps-regression")
svm.fit(131, type = "eps-regression", feature = "sc")
svm.fit(131, type = "eps-regression", feature = "fc")
```
