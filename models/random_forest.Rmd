  ...
  if(feature == "fc"){
   rf_classifier = randomForest(Depressive_Ep ~ FCfeature_1+FCfeature_2+FCfeature_3, data=df_train, ntree=100, mtry=2, importance=TRUE)
    ...
    rf.pred = predict(rf_classifier, df_test[,c(1:3)])
     ...
     rf_classifier = randomForest(Depressive_Ep~FCfeature_1+FCfeature_2+FCfeature_3+SCfeature_1+SCfeature_2+SCfeature_3, data = df_train, ntree=100, importance=TRUE)
    summary(rf_classifier)
...
dt.fit = function(index_trait, feature = "all"){
  ...
  data = df_train,method="classification")
    summary( decision_tree)
    dt.pred = predict(decision_tree, df_test[,c(1:3)])
  }else if(feature == "sc"){
     decision_tree = rpart(Depressive_Ep~SCfeature_1+SCfeature_2+SCfeature_3, data = df_train, method="classification")
    summary(rf_classifier)
   dt.pred = predict(decision_tree, df_test[,c(1:3)])
  }else{
     decision_tree = randomForest(Depressive_Ep~FCfeature_1+FCfeature_2+FCfeature_3+SCfeature_1+SCfeature_2+SCfeature_3, data = df_train, method="classification")
    summary(decision_tree)
